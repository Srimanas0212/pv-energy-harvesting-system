...
The PV cell's voltage and current outputs feed the MPPT control algorithm; PV current is sensed via a small sensing resistor. The model was implemented and validated in the Cadence environment (see `images/fig3.1-pv-cell-model.png` and `images/fig3.2-pv-output-characteristics.png`).
...
Charge is moved between capacitors via switching action, using complementary clock signals for efficient energy transfer (see `images/fig3.3-voltage-doubler-circuit.png`).
...
**Result**: an input voltage of ~2 V is boosted to ~3.27 V using complementary clock signals (see `images/fig3.4-voltage-doubler-output.png`).
